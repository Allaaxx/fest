...
import { X, Send, Paperclip, Smile, Phone, Video, MoreVertical, Star, Store, Clock } from "lucide-react"
...
export default function ChatModal({ isOpen, onClose, vendor, product }: ChatModalProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: `Olá! Vi que você tem interesse no ${product.name}. Como posso ajudá-lo?`,
      sender: "vendor",
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-4 text-sm text-gray-500">
                  <span className={vendor.isOnline ? "text-green-600" : ""}>
                    {vendor.isOnline ? "Online" : `Visto por último ${vendor.lastSeen}`}
                  </span>
                  <div className="flex items-center space-x-1">
                    <Clock className="h-3 w-3" />
                    <span>Responde em {vendor.responseTime}</span>
                  </div>
                </div>
              </div>
            </div>
